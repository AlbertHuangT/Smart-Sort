...
The Trash 是一个垃圾分类与环保互动产品仓库，当前只维护 React Native 客户端。

## 仓库结构

```text
.
├── the-trash-rn/                  # Expo / React Native 主工程
...
`project-ref` 为 Supabase 项目短 ID（Dashboard URL 中 `project/<ref>`）。
...
- `make migrations-sync` 仅保留兼容入口，当前为 no-op。
...
- `Authentication with Apple Developer Portal failed / no team`
  - iOS 云构建不可用时，可先走本地真机安装或 Android 开发流程。
