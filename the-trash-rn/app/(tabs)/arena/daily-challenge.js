...
export default function DailyChallengeScreen() {
  ...
  const incrementDailyChallenge = useArenaStore((state) => state.incrementDailyChallenge);
  ...
  return (
    <ScreenShell title="每日挑战">
      <View className="rounded-3xl border border-white/10 bg-white/5 p-6 mb-4">
        ...
        <Text className="text-white text-2xl font-semibold mb-1">{dailyChallenge.prompt}</Text>
        <Text className="text-white/60 text-sm mb-3">
          进度 {dailyChallenge.progress}/{dailyChallenge.total} · {progressPercent}%
        </Text>
        <Text className="text-brand-neon text-xs">奖励 {dailyChallenge.reward ?? '待公布'}</Text>
      </View>
      <TrashButton
