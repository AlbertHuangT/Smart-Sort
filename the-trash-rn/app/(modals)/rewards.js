  ...
  return (
    <ModalSheet title="奖励">
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <Text className="text-white/60 text-xs">当前版本暂未开放积分兑换。</Text>
        )}
        renderItem={({ item }) => {
          ...
          return (
            <View className="rounded-3xl border border-white/10 p-4 mb-3">
              ...
              <Text className="text-white/60 text-xs mb-2">{item.points} 分</Text>
              <Pressable
                onPress={() => redeem(item.id)}
                disabled={disabled}
                className="rounded-2xl py-2 items-center"
                style={{
                  backgroundColor: disabled ? 'rgba(255,255,255,0.1)' : '#32f5ff'
                }}
              >
