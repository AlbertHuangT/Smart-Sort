  ...
  return (
    <ModalSheet title="升级账号">
      <Text className="text-white/70 text-sm mb-4">
        游客进度只保存在本地。升级后可同步到 Supabase，并开启对战、排行榜等功能。
      </Text>
      <TrashInput label="邮箱" value={email} onChangeText={setEmail} placeholder="you@example.com" />
      <TrashInput
        label="密码"
        value={password}
      ...
      <TrashButton title={loading ? '提交中…' : '使用邮箱注册'} onPress={handleUpgrade} disabled={loading} />
    </ModalSheet>
  );
}
